...
@implementation HOMEUUIDKeyChain
...
+ (NSString *)Value
{
    ...
    AKeyChainUUIDSessionCache = [HOMEUUIDKeyChain getKeyChainUUID];
    ...
    [KeyChainUUIDPairs setObject:[HOMEUUIDKeyChain makeUUID] forKey:aKeyChainUUIDKey];
    [HOMEUUIDKeyChain saveObject:KeyChainUUIDPairs];
...
+ (NSString *)makeUUID
{
    ...
    NSString *BundleIdentifier = [NSString stringWithFormat:@"%zd%@%f", (NSUInteger)(arc4random() % NSUIntegerMax),[infos objectForKey:@"CFBundleIdentifier"],now];
...
+ (void)Renew
{
    [HOMEUUIDKeyChain deleteKeychain:aKeyChainUUIDIdentifier];
}

+ (NSString *)getKeyChainUUID
{
    NSMutableDictionary *KeyChainUUIDPairs = (NSMutableDictionary *)[HOMEUUIDKeyChain loadKeychainData:aKeyChainUUIDIdentifier];
...
+(void)saveUserInfo:(id)userInfo
{
    NSMutableDictionary *keychainQuery = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                          (__bridge id)kSecClassGenericPassword,(__bridge id)kSecClass,
                                          BUNDLE_IDENTIFIER, (__bridge id)kSecAttrService,
                                          BUNDLE_IDENTIFIER, (__bridge id)kSecAttrAccount,
                                          (__bridge id)kSecAttrAccessibleAfterFirstUnlock,(__bridge id)kSecAttrAccessible,
                                          nil];
    SecItemDelete((__bridge CFDictionaryRef)keychainQuery);
